...
def volume(nodes, graph):
    ...
    for node in nodes:
        for edge in graph.edges():
            if((edge[0] in node) or edge[1] in node):
                if(edge not in seen):
                    volume += 1
                    seen.append(edge)
    return volume    
...
def norm_cut(S, T, graph):
    ...
    norm_cut = (cut_set / vol_S) + (cut_set / vol_T)
    return norm_cut
    pass
...
def brute_force_norm_cut(graph, max_size):
    ...
    result_tuple =[]
    edges = graph.edges()
    for edge_set_size in range(1, max_size+1):
        for edge in combinations(edges, edge_set_size):
            graph_copy = graph.copy()
            graph_copy.remove_edges_from(edge)
            components = get_components(graph_copy)
            if (len(components) == 2):
                score = norm_cut(components[0], components[1], graph)
                result_tuple.append((score, list(edge)))
    return result_tuple                
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        partitioned_graph = partition_girvan_newman(graph, depth)
        score.append((depth, norm_cut(partitioned_graph[0].nodes(), partitioned_graph[1].nodes, graph)))
    return score
    pass
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if ((n not in neighbors) and (n != node)):
            ...
            jaccard_coeff = (len(neighbors & neighbors2)) / (len(neighbors | neighbors2))
            recommended_edges.append(((node, n), jaccard_coeff))
    
    recommended_edges =  sorted(recommended_edges, key = lambda x:(-x[1], x[0][1]))
    return recommended_edges[:k]
    pass
...
def evaluate(predicted_edges, graph):
    ...
    for edge in predicted_edges:
        if graph.has_edge(*edge) == True:
            predicted_value += 1
    eval_value = predicted_value / len (predicted_edges)
    return eval_value
    pass
