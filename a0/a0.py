...
def print_num_friends(users):
    ...
    for user in sorted_users:
        print("{0} {1}".format(user['screen_name'], len(user['friends'])))
    pass
...
def friend_overlap(users):
    ...
    """
    ###TODO
    friend_overlap_tuple = []
    for i in range(len(users)):
        for j in range(i+1, len(users)):
            shared_accounts = set(users[i]['friends']) & set(users[j]['friends'])
            friend_overlap_tuple.append((users[i]['screen_name'], users[j]['screen_name'], len(shared_accounts)))
    return sorted(friend_overlap_tuple, key=lambda x: x[2], reverse = True)    
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    common_friends = set(hillary['friends']) & set(donald['friends'])
    followed = robust_request(twitter, 'users/lookup', {'user_id': common_friends})

    followed_screen_names =[]
    for name in followed:
        followed_screen_names.append(name['screen_name'])

    return followed_screen_names
    pass
...
def draw_network(graph, users, filename):
    ...
    candidate_labels ={}
    for user in users:
    	candidate_labels[user['screen_name']] = user['screen_name']

    figure = plt.figure()
    nx.draw_networkx(graph, labels = candidate_labels, node_size =20, width =0.1, alpha = 0.6)
    plt.axis('off')
